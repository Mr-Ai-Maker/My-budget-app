...
// ===============================
// المحاسب الذكي (مساعد قائم على قواعد)
// ===============================
...
return "لا توجد بيانات كافية بعد لتحليل وضعك المالي. أضف بعض العمليات أولاً 🙂";
...
advice.push(`💸 مصروفاتك تمثل <b>${Math.round(expenseRatio)}%</b> من دخلك، وهذه نسبة مرتفعة. حاول تقليلها تدريجياً إلى أقل من 50%.`);

}else if(expenseRatio>50){

advice.push(`⚠️ مصروفاتك تمثل <b>${Math.round(expenseRatio)}%</b> من دخلك. لا بأس بها لكن يمكن تحسينها.`);

}else{

advice.push(`✅ مصروفاتك تمثل فقط <b>${Math.round(expenseRatio)}%</b> من دخلك، وهذا مؤشر جيد على التوازن المالي.`);
...
expenseByCategory[item.category] = (expenseByCategory[item.category]||0) + Number(item.amount);
...
if(expenseByCategory[cat]>topAmount){

topAmount = expenseByCategory[cat];
...
advice.push(`🏆 أعلى تصنيف صرف عندك هو <b>${topCat}</b> بمبلغ ${topAmount} ريال. راجع مصروفاتك فيه إذا كنت تريد التوفير.`);

}

if(totals.investment<=0){

advice.push(`🌱 لا توجد لديك استثمارات مسجلة بعد. حتى مبلغ صغير شهرياً في الاستثمار يصنع فرقاً كبيراً على المدى الطويل.`);

}else{

advice.push(`🌱 لديك ${totals.investment} ريال في الاستثمارات، استمر في ذلك بانتظام.`);

}

if(totals.commitment>0){

advice.push(`📌 لديك التزامات بقيمة ${totals.commitment} ريال قائمة، حاول سدادها تدريجياً لتقليل الأعباء المستقبلية.`);

}

if(totals.balance<0){

advice.push(`🚨 رصيدك الحالي بالسالب (${totals.balance} ريال). ينصح بمراجعة مصروفاتك فوراً وتقليل غير الضروري منها.`);
...
if(lower.includes("احذف") && (lower.includes("اخر") || lower.includes("آخر"))){

if(transactions.length===0){

appendAssistantMessage("لا توجد عمليات لحذفها 🤷", "bot");
...
appendAssistantMessage(`🗑️ تم حذف آخر عملية: "${last.description}" بمبلغ ${last.amount} ريال.`, "bot");
...
if(lower.includes("نصيح") || lower.includes("توفير") || lower.includes("توازن")){

appendAssistantMessage(generateSavingAdvice(), "bot");

return;

}

// استعلام عن الرصيد / الملخص

if(lower.includes("رصيد") || lower.includes("ملخص") || lower.includes("كم عندي") || lower.includes("وضعي المالي")){

const totals = getTotals();

appendAssistantMessage(

`💼 الرصيد الحالي: ${totals.balance} ريال<br>`+

`💰 الدخل (منذ آخر تقرير): ${totals.periodIncome} ريال<br>`+

`💸 المصروفات: ${totals.expense} ريال<br>`+

`🌱 الاستثمارات: ${totals.investment} ريال<br>`+

`📌 الالتزامات: ${totals.commitment} ريال`,
...
// إضافة عملية: البحث عن مبلغ رقمي + نوع العملية

const amountMatch = text.match(/\d+(\.\d+)?/);

if(amountMatch && (lower.includes("أضف") || lower.includes("اضف") || lower.includes("سجل") || lower.includes("دخل") || lower.includes("مصروف") || lower.includes("استثمار") || lower.includes("التزام"))){

const amount = Number(amountMatch[0]);

let type = "expense";

if(lower.includes("دخل") || lower.includes("راتب")){

type = "income";

}else if(lower.includes("سحب استثمار") || lower.includes("سحب") && lower.includes("استثمار")){

type = "investment_withdraw";

}else if(lower.includes("استثمار")){

type = "investment";

}else if(lower.includes("سداد") && lower.includes("التزام")){

type = "commitment_pay";

}else if(lower.includes("التزام")){

type = "commitment";

}else if(lower.includes("مصروف") || lower.includes("صرف")){
...
const description = text.replace(/\d+(\.\d+)?/, "").trim() || "عملية من المحاسب الذكي";
...
appendAssistantMessage(`✅ تمت إضافة العملية: "${description}" بمبلغ ${amount} ريال ضمن تصنيف ${category}.`, "bot");
...
"لم أفهم طلبك 🤔. جرّب مثلاً:<br>"+

"• \"أضف مصروف 50 ريال طعام\"<br>"+
