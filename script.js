...
let commitmentAdded = 0;

let investmentWithdrawn = 0;

let commitmentPaid = 0;

let periodIncome = 0;

let periodExpenseOriginal = 0;

let periodInvestment = 0;

let periodCommitment = 0;

let periodInvestmentWithdrawn = 0;

let periodCommitmentPaid = 0;
...
if(inPeriod){ periodExpenseOriginal += amt; }
...
investmentWithdrawn += amt;

if(inPeriod){ periodInvestment -= amt; periodInvestmentWithdrawn += amt; }
...
if(inPeriod){ periodCommitment -= amt; periodCommitmentPaid += amt; }
...
const expense = expenseOriginal + investmentWithdrawn + commitmentPaid;

const periodExpense = periodExpenseOriginal + periodInvestmentWithdrawn + periodCommitmentPaid;
...
income - expenseOriginal - investmentAdded - commitmentAdded
...
"طعام":["طعام","اكل","أكل","مطعم","غداء","غدا","عشاء","فطور","فطار","وجبة","برجر","بيتزا","كيك","حلا","حلى","قهوة","كافيه","مقهى","بوفيه","شاورما","بيك أب","مندي","دجاج"],

"مواصلات":["مواصلات","بنزين","وقود","تاكسي","تكسي","اوبر","أوبر","كريم","سيارة","تكس","مشوار","صيانة سيارة","تكسي","باص","مترو","قطار","طيران","تذكرة سفر"],

"سكن":["سكن","ايجار","إيجار","شقة","بيت","كهرباء","ماء","صيانة منزل","عمارة","فيلا"],

"فواتير":["فاتورة","فواتير","اشتراك","انترنت","إنترنت","جوال","اتصالات","stc","موبايلي","زين","واي فاي"],

"تسوق":["تسوق","ملابس","بقالة","سوبرماركت","بقاله","سوق","مشتريات","امازون","أمازون","نون","حذاء","شنطة","اكسسوار"],

"ترفيه":["ترفيه","سينما","لعبة","العاب","ألعاب","اشتراك ترفيه","نتفلكس","شاهد","بلايستيشن","رحلة","نزهة"],

"راتب":["راتب","معاش","دخل شهري","مكافأة","بونص"],

"استثمار":["استثمار","اسهم","أسهم","صندوق","محفظة","تداول","عملات","ذهب"],

"التزامات":["التزام","التزامات","قسط","سداد","دين","قرض","تمويل"]
...
alert("تعذر إيجاد هذه العملية (ربما حُذفت مسبقاً أو تحتاج تحديث الصفحة).");
...
`هل تريد حذف "${item.description}" بمبلغ ${item.amount} ريال؟`,
...
transactions.filter(item=>item.id>=periodStart).length;
...
`سيتم حفظ تقرير "${label}" وبدء فترة جديدة (الدخل والمصروفات والاستثمار والالتزامات لهذا الشهر تبدأ من الصفر، والرصيد الحالي يبقى كما هو). هل تريد المتابعة؟`,
...
expense: totals.periodExpense,

investment: totals.periodInvestment,

commitment: totals.periodCommitment,

balance: totals.balance,

count: transactions.filter(item=>item.id>=periodStart).length
...
totals.periodExpense,

Math.max(totals.periodInvestment,0),

Math.max(totals.periodCommitment,0)
...
if(statsExpenseEl) statsExpenseEl.textContent = totals.periodExpense + " ريال";

if(statsInvestmentEl) statsInvestmentEl.textContent = totals.periodInvestment + " ريال";

// تفصيل المصروفات حسب التصنيف (أين ذهبت أموالي) — لهذه الفترة فقط
...
if(item.type==="expense" && item.id>=periodStart){
...
const trendExpenseData = lastReports.map(r=>r.expense).concat([totals.periodExpense]);
...
const diff = lastReport.expense - totals.periodExpense;
...
(totals.periodExpense/monthlyBudget)*100,
...
const percent=(totals.periodExpense/monthlyBudget)*100;
